...
test.serial(
  `processes incoming transaction.processed event, do not deliver to destination when usage code does not match`,
  async (t) => {
...
test.serial(
  `processes incoming transaction.processed event, deliver to destination when usage code matches`,
  async (t) => {
    // loading incoming EDI file from S3
    buckets.on(GetObjectCommand, {}).resolves({
      body: sdkStreamMixin(
